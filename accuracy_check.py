...
with open('Genome_probabilities.txt') as json_file:
    ...
    with open('query_seq300_8.json') as json_file:
        ...
        for og_seq in seqs:   #og_seq will hold the original sequence, so loops 12 times
            mod_seqs = seqs[og_seq]
            base_seq_num = base_seq_num + 1
            mod_num = 0

            for modded in mod_seqs:   #modded holds each modded sequence as a string
                ...
                for object in list_of_objects:
                    ...
                    for nuc in predicted_alignment:
                        curr = data[i]
                        keep = 0
                        hold = 'A'
